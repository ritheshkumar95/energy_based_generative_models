...
def _get_adapted_dataset(split):
    ...
    if split != "train":
        dataset[key_img], dataset[key_lbl] = _adapt(dataset[key_img], dataset[key_lbl])
...
def _get_dataset():
    ...
    df = pd.read_csv(
        "/network/tmp1/kumarrit/data/kddcup.data_10_percent_corrected",
        header=None,
        names=col_names,
    )
    text_l = [
    ...
    x_train = x_train[y_train != 1]  # labels 0 (not-normal)
    y_train = y_train[y_train != 1]  # labels 0 (not-normal)
...
def _to_xy(df, target):
    ...
    dummies = df[target]
    return (
        df.as_matrix(result).astype(np.float32),
        dummies.as_matrix().astype(np.float32),
    )
